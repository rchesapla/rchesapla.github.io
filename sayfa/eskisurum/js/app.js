...
$scope.calculateROI = function () {
  ...
  // Günlük USD kazancı al - Burada manuel formül kullandık
  const powerInGhs = convertHashrate($scope.formData.power, $scope.formData.unit, 'GH/s');
  const netPowerInGhs = convertHashrate($scope.formData.networkPower, $scope.formData.networkUnit, 'GH/s');
  const userPowerPercentage = powerInGhs / netPowerInGhs;
  const blockTime = $scope.formData.timeUnit === 'minutes' ? $scope.formData.blockTime * 60 : $scope.formData.blockTime;
  const earningsPerDay = ($scope.formData.blockSize * userPowerPercentage) * (86400 / blockTime);
  const dailyUSD = $scope.formData.currency.in_game_only ? 0 : earningsPerDay * exchangeRates[$scope.formData.currency.name]['usd'];
